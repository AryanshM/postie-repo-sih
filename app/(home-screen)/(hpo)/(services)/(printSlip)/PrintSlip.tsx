...
const PrintSlip = () => {
  ...
  const banners = [
    { source: images.hpo_printSlip1 },
    { source: images.hpo_printSlip2 },
    { source: images.hpo_printSlip3 },
  ];
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setConsignmentID("");
        router.push({
          pathname: "/(home-screen)/(hpo)/(services)/(printSlip)/Details",
          params: {
            ...response.data,
  ...
  return (
    <View className="flex-1 px-1 bg-[#F8FAFC]">
      ...
      <View className="items-center mt-8">
        ...
        <TouchableOpacity
          onPress={() =>
            router.replace("/(home-screen)/(hpo)/(services)/(book-consigmnet)/ArticleDetails")
          }
          className="mt-6"
        >
