...
export default function Scan() {
  ...
  const onBarcodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      const response = await api.post("/employee/secondary_sort/", {
       "consignment_id": Number(data),
      });

      if (response.status === 200) {

        setSorting(response.data);
        setQrResult(data);
  ...
  if (!permission) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50">
        <Headers heading="Check-In Cartons" back={() => router.replace("/(home-screen)/(ich)/(tabs)/Home")} />
        <Text className="text-2xl font-bold text-primaryColor mb-4">Cartons</Text>
  ...
  return (
    <View className="flex-1 bg-screenBackgroundColor px-4">
      <Headers
        heading="Sorting"
        back={() => router.replace("/(home-screen)/(ich)/(tabs)/Home")}
      />
    ...
    {sorting ? (
      <View className="bg-white rounded-lg shadow-md p-4 mb-4">
        <Text className="text-gray-950 font-bold text-lg">
          Consignment ID: {sorting.consignment_id}
        </Text>
        <Text className="text-gray-700 text-base">
          Office ID: {sorting.office_id}
        </Text>
        <Text className="text-gray-700 text-base">
          Office Name: {sorting.office_name}
        </Text>
      </View>
    ) : (
      <Text className="text-gray-500 text-center">
        No sorting details available yet.
      </Text>
    )}
  </View>
</ScrollView>
