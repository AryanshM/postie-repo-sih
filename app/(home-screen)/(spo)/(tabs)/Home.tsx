...
const Home = () => {
  ...
  const handleComplaints = () => {
    router.push("/(home-screen)/(spo)/(services)/(complaints)/ComplaintStatus");
  };

  const handlePrintSlip = () => {
    router.push("/(home-screen)/(spo)/(services)/(printSlip)/PrintSlip");
  };

  const handleCarton = () => {
    router.push("/(home-screen)/(spo)/(services)/(create-carton)/Create");
  };

  const handleCheckIn = () => {
    router.push("/(home-screen)/(spo)/(services)/(check-in)/CheckIn");
  };

 

  const handleCheckOutHPO = () => {
    router.push("/(home-screen)/(spo)/(services)/(check-out)/CheckOutHPO");
  };
  ...
  return (
    <View className="flex-1 bg-primaryColor">
        ...
        <View className="mt-4 flex flex-row p-4 justify-between">
          ...
          <View className="flex-row items-center flex-1">
            ...
            <View>
              ...
              {/* Phone Number below Greeting */}
              <Text
                className="text-white "
                style={{ fontSize: isTablet ? 15 : 12 }}
              > <Text className="font-bold">SPO </Text>
     ...
     <View className="flex-1 bg-screenBackgroundColor -mt-[7%] rounded-tl-[7%] pt-6 shadow-md">
     <ScrollView className="rounded-3xl" contentContainerStyle={{ flexGrow: 1 }}>
