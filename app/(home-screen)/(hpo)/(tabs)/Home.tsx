...
const Home = () => {
  ...
  const handleComplaints = () => {
    router.push("/(home-screen)/(hpo)/(services)/(complaints)/ComplaintStatus");
  };

  const handlePrintSlip = () => {
    router.push("/(home-screen)/(hpo)/(services)/(printSlip)/PrintSlip");
  };

  const handleCarton = () => {
    router.push("/(home-screen)/(hpo)/(services)/(create-carton)/Create");
  };

  const handleCheckIn = () => {
    router.push("/(home-screen)/(hpo)/(services)/(check-in)/CheckIn");
  };

  const handleCheckOutHPO = () => {
    router.push("/(home-screen)/(hpo)/(services)/(check-out)/CheckOutHPO");
  };
  const handleSorting = () => {
    router.push("/(home-screen)/(hpo)/(services)/(secondary-sorting)/Scan");
  }
    ...
    <View className="flex-1 bg-primaryColor">
      ...
      <View className="h-[23%] w-full bg-primaryColor shadow-lg">
        <View className="mt-4 flex flex-row p-4 justify-between">
          ...
          <View className="flex-row items-center flex-1">
            ...
            <View>
              ...
              > <Text className="font-bold">HPO </Text>
                {data?.city_district} {data?.state}
                <Text>{" - "}</Text>
                <Text>{String(data?.pincode)}</Text>
              </Text>
            </View>
        ...
        <View className="items-center w-full mt-3">
              <Text className="text-center text-white text-xl font-semibold">All-In-One Access</Text>
              <Text className="text-center text-white text-xl font-semibold">Simplify Your Day!</Text>
