...
const CheckIn = () => {
  ...
  return (
    <View className="flex-1 bg-screenBackgroundColor px-4">
      {/* Header */}
      <Headers heading="Check-Out Cartons" back={() => router.replace("/(home-screen)/(ich)/(tabs)/Home")} />
      ...
      <View className="items-center my-8">
        <TouchableOpacity
          onPress={() => router.replace("/(home-screen)/(ich)/(services)/(check-out)/ScanHPO")}
        ...
        <TouchableOpacity
        onPress={() => {
          router.replace("/(home-screen)/(ich)/(services)/(check-out)/ScanHPO");
        }}
