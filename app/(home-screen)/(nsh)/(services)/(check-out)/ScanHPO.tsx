...
export default function Scan() {
  ...
  const onBarcodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      const response = await api.post("/employee/checkout_from_nsh/", {
  ...
  if (!permission) {
    return (
      <View className="flex-1 items-center justify-center bg-screenBackgroundColor">
        <Headers heading="Check-In Cartons" back={() => router.replace("/(home-screen)/(nsh)/(services)/(check-out)/CheckOutHPO")} />
