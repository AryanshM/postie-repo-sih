...
export default function Scan() {
  ...
  const onBarcodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      const response = await api.post("/employee/checkout/", {
  ...
  if (!permission) {
    return (
      <View className="flex-1 items-center justify-center bg-screenBackgroundColor">
        <Headers heading="Check-out Cartons" back={() => router.replace("/(home-screen)/(hpo)/(services)/(check-out)/CheckOutHPO")} />
