...
const Home = () => {
  ...
  const handleComplaints = () => {
    router.push("/(home-screen)/(ich)/(services)/(complaints)/ComplaintStatus");
  };

  const handlePrintSlip = () => {
    router.push("/(home-screen)/(ich)/(services)/(printSlip)/PrintSlip");
  };

  const handleCarton = () => {
    router.push("/(home-screen)/(ich)/(services)/(create-carton)/Create");
  };

  const handleCheckIn = () => {
    router.push("/(home-screen)/(ich)/(services)/(check-in)/CheckIn");
  };

  const handleCheckOutHPO = () => {
    router.push("/(home-screen)/(ich)/(services)/(check-out)/CheckOutHPO");
  };
  const handleSorting = () => {
    router.push("/(home-screen)/(ich)/(services)/(secondary-sorting)/Scan");
  }
        ...
        <View className="mt-4 flex flex-row p-4 justify-between">
          ...
          <View className="flex-row items-center flex-1">
            ...
            <View>
              ...
              > <Text className="font-bold">ICH </Text>{data?.city_district}  {data?.state}
                <Text>{" - "}</Text>
