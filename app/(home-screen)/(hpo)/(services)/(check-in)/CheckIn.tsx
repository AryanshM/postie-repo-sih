...
const CheckIn = () => {
  ...
  useEffect(() => {
    const fetchCartons = async () => {
      setLoading(true);
      try {
        const response = await api.get("/employee/get_checkin_container/");
  ...
  return (
    <View className="flex-1 bg-screenBackgroundColor px-4">
      {/* Header */}
      <Headers heading="Check-In Cartons" back={() => router.replace("/(home-screen)/(spo)/(tabs)/Home")} />
      ...
      <View className="items-center my-8">
        <TouchableOpacity
          onPress={() => router.replace("/(home-screen)/(hpo)/(services)/(check-in)/Scan")}
          className="h-24 w-24 bg-white rounded-full shadow-md items-center justify-center"
        >
          <Image
      ...
      {filtered.length === 0 ? (
        ...
        <TouchableOpacity
        onPress={() => {
          router.replace("/(home-screen)/(hpo)/(services)/(check-in)/Scan");
        }}
