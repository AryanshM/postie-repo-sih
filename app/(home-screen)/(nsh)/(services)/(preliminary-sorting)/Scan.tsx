...
export default function Scan() {
  ...
  const onBarcodeScanned = async ({ data }: BarcodeScanningResult) => {
    ...
    try {
      const response = await api.post("/employee/preliminary_sort/", {
  ...
  if (!permission) {
    return (
      <View className="flex-1 items-center justify-center bg-gray-50">
        <Headers heading="Check-In Cartons" back={() => router.replace("/(home-screen)/(nsh)/(tabs)/Home")} />
