...
const CheckIn = () => {
  ...
  return (
    <View className="flex-1 bg-screenBackgroundColor px-4">
      {/* Header */}
      <Headers heading="Check-Out Cartons" back={() => router.replace("/(home-screen)/(hpo)/(tabs)/Home")} />
      ...
      <View className="items-center my-8">
        <TouchableOpacity
          onPress={() => router.replace("/(home-screen)/(hpo)/(services)/(check-out)/ScanHPO")}
        ...
        <TouchableOpacity
        onPress={() => {
          router.replace("/(home-screen)/(hpo)/(services)/(check-out)/ScanHPO");
        }}
