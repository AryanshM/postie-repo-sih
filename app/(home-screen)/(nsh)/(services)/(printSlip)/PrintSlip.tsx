...
const PrintSlip = () => {
  ...
  const banners = [
    { source: images.NSH_printSlip1 },
    { source: images.NSH_printSlip2 },
    { source: images.NSH_printSlip3 },
  ];
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setConsignmentID("");
        router.push({
          pathname: "/(home-screen)/(nsh)/(services)/(printSlip)/Details",
          params: {
            ...response.data,
  ...
  return (
    <View className="flex-1 px-1 bg-[#F8FAFC]">
      ...
      <View className="items-center mt-8">
        ...
        {/* <TouchableOpacity
          onPress={() =>
            router.replace("/(home-screen)/(nsh)/(services)/(book-consigmnet)/ArticleDetails")
          }
          className="mt-6"
        >
